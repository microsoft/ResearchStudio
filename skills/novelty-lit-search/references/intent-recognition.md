...
Goal: turn the user's free-text input into 3-5 search queries (map mode) or 3-5 signature terms (collision mode).
...
```
You read a user's research question and extract 3-5 search queries to send to academic search APIs.
...
- Query 4 (optional): APPLICATION-ANGLE — domain or use-case, ~3-5 words. Example: "text-to-image generation".
- Query 5 (optional): VENUE-INSIDER — phrase a top-conference reviewer would use, ~5-8 words.
