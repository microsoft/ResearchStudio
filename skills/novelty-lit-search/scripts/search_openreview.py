...
I/O:
  python -m scripts.search_openreview --queries '["..."]' --window-months 6 --out hits.json
"""
...
def search(client, query: str, since: datetime, venues: list[str], max_results: int = 50) -> list[dict]:
    """Use the v2 client's full-text-like search by listing recent submissions
    in active venues and filtering by query relevance.

    OpenReview doesn't have a true free-text search endpoint that's stable across
    venues — this function trades exhaustiveness for stability by pulling the most
    recent submissions of a few in-flight venues and filtering."""
    out = []
    ...
    for venue_id in venues:
        ...
        try:
            ...
            print(f'  openreview {venue_id}: {e}', file=sys.stderr); continue
        for n in notes[:300]:  # cap per venue to keep it bounded
...
def main():
    ...
    until = now - timedelta(days=30 * args.window_min_months) if args.window_min_months > 0 else None
    ...
    for q in queries:
        try:
            hits = search(client, q, since, venues=venues, max_results=args.max_per_query)
        except Exception as e:
            print(f'  openreview {q!r} failed: {e}', file=sys.stderr); continue
        for h in hits:
            # Apply window_min_months filter client-side using cdate already checked in search()
            # (search() filters >= since; here we additionally filter <= until if set)
            if until is not None:
                # We don't have cdate in the dict, so skip until-filter for openreview
                # (window-min-months for openreview is rarely needed in the new design)
                pass
            key = h['title_norm']
