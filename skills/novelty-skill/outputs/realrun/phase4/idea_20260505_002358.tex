...
\section*{Innovation patterns active in this area}
\textbf{Based on Phase 0 retrieval} (n = 8 on-topic papers):
\begin{itemize}[leftmargin=*]
  \item \texttt{assumption\_audit\_and\_relax}: 4 papers (50\%) \quad saturation: \emph{high}
  \item \texttt{analytic\_refinement\_of\_loose\_bounds}: 2 papers (25\%) \quad saturation: \emph{medium}
  \item \texttt{auxiliary\_signal\_engineering}: 2 papers (25\%) \quad saturation: \emph{medium}
  \item \texttt{formal\_reframing\_via\_equivalence}: 1 papers (13\%) \quad saturation: \emph{low}
  \item \texttt{evaluation\_validity\_via\_controlled\_perturbation}: 1 papers (13\%) \quad saturation: \emph{low}
  \item \texttt{mechanistic\_probing\_with\_intervention}: 1 papers (13\%) \quad saturation: \emph{low}
  \item \texttt{root\_cause\_surgical\_remediation}: 1 papers (13\%) \quad saturation: \emph{low}
\end{itemize}

\textbf{This candidate uses:}
\begin{itemize}[leftmargin=*]
  \item \texttt{analytic\_refinement\_of\_loose\_bounds} (primary, saturation in this area: \emph{medium})
  \item \texttt{assumption\_audit\_and\_relax} (secondary, saturation in this area: \emph{high})
\end{itemize}

\textbf{Position.} The candidate's primary pattern (analytic\_refinement) is at medium saturation in this area, while the secondary (audit) is at high saturation. The audit secondary shares the saturated class but explicitly targets the shared limitation of audit-class papers in this area --- heuristic / iterative / disagreement-based regularization schedules --- by deriving a finite-sample form of the SFT-as-adversarial equivalence. The substantive contribution differentiates from existing audit-class work by producing a derived rather than heuristic schedule.
