...
def next_step(run_dir: Path, root: Path, query: str | None = None) -> int:
    ...
    if not (p0 / 'lit_table.md').exists():
        return _emit('Papers retrieved; lit_table.md not yet written.',
                     ...
                     notes='Pure classification — delegate to a CHEAP/FAST sub-agent (e.g. '
                           'Haiku-class or effort=low): tag each paper with 1-3 of the 15 '
                           'patterns + bottleneck + open_issue + retrieved_via per the rubric. '
                           'Routing signal: none (just the file).',
                     run_dir=d)
    ...
    if not p1.exists():
        return _emit('Phase 0 complete.', 'Phase 1 — bottleneck identification', 'llm_subagent',
                     ...
                     inputs=['the user query + intake context',
                             str(p0 / 'lit_table.md'),
